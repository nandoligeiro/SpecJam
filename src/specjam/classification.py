...
def classify_request(text: str, *, ambiguous: bool = False, critical: bool = False) -> Classification:
    ...
    if not normalized:
        return Classification("L3", "delivery", "empty requests are ambiguous and require discovery")
    ...
    if ambiguous or critical or any(word in normalized for word in ("security", "payment", "migration", "breaking", "production")):
        return Classification("L3", "delivery", "ambiguous or critical work requires discovery and full gates")
